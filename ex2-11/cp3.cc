...
# define SAMEFILE "cp: %s and %s are identical (not copied).\n"
# define FILENOTEXIST "cp: %s: No such file\n"
...
void copy_file(const char * src, const char * dst){
    ...
    if(!is_file_exist(src)){
        fprintf(stderr,FILENOTEXIST,src);
        exit(1);
    }

    if(!is_file_exist(dst)){
        fprintf(stderr,FILENOTEXIST,dst);
        exit(1);
    }

    if(strcmp(src,dst) == 0){
        fprintf(stderr,SAMEFILE,src,dst);
        exit(1);
    }
